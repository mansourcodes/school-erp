@extends('backend.layouts.' . $print)


@section('content')
    @foreach ($classRooms as $classRoom)
        @foreach ($classRoom->long_name as $class_room_long_name)
            <div class="page">

                @if (Setting::get('print_header'))
                    <img class="w-100" src="{{ URL::asset(Setting::get('print_header')) }}" />
                @endif


                <h3 class="title text-center">
                    {{ empty(Setting::get('monthly_exams_template.title')) ? __('reports.monthly_exams_template') : Setting::get('monthly_exams_template.title') }}
                </h3>

                <h3 class="title text-center">
                    {{ $course->long_name }}
                </h3>

                {!! Setting::get('monthly_exams_template.pre') !!}

                <table class="table table-striped table-bordered">
                    <tbody>
                        <tr>
                            <td>
                                {{ $class_room_long_name }}</td>
                        </tr>
                    </tbody>
                </table>


                <table style="table-layout: auto;" class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            <th width="1%">#</th>
                            <th width="20%">Ø§Ø³Ù… Ø§Ù„Ø·Ø§Ù„Ø¨</th>
                            <th>Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹
                                <br>
                                (30 Ø¯Ø±Ø¬Ø©)
                            </th>
                            <th>Ø§Ù„ÙˆÙ‚ÙØ© Ø§Ù„ØªÙ‚ÙˆÙŠÙ…ÙŠØ© Ø§Ù„Ø£ÙˆÙ„Ù‰ (10 Ø¯Ø±Ø¬Ø§Øª)
                                <br>
                                Ø§Ù„ØªØ§Ø±ÙŠØ® : ..../.... /....
                            </th>
                            <th>Ø§Ù„ÙˆÙ‚ÙØ© Ø§Ù„ØªÙ‚ÙˆÙŠÙ…ÙŠØ© Ø§Ù„Ø«Ø§Ù†ÙŠØ© (10 Ø¯Ø±Ø¬Ø§Øª)
                                <br>
                                Ø§Ù„ØªØ§Ø±ÙŠØ® : ..../.... /....
                            </th>
                            <th>Ø§Ù„ÙˆÙ‚ÙØ© Ø§Ù„ØªÙ‚ÙˆÙŠÙ…ÙŠØ© Ø§Ù„Ø«Ø§Ù„Ø«Ø© (10 Ø¯Ø±Ø¬Ø§Øª)
                                <br>
                                Ø§Ù„ØªØ§Ø±ÙŠØ® : ..../.... /....
                            </th>
                        </tr>
                    </thead>
                    <tbody>

                        @foreach ($classRoom->students as $key => $student)
                            <tr>
                                <td>{{ ++$key }}</td>
                                <td>
                                    {{ $student->name }}
                                </td>
                                <td></td>
                                <td></td>
                                <td></td>
                                <td></td>
                            </tr>
                        @endforeach
                    </tbody>


                    </tbody>
                </table>





                {!! Setting::get('monthly_exams_template.pro') !!}

            </div>

            @if (!$loop->parent->last)
                <div class="new-page"></div>
            @endif
        @endforeach
    @endforeach
@endsection
